...
define('KALTURA_CATEGORIES_ROOT_ID',	'6404482');
define('KALTURA_DEFAULT_CATEGORY',	'MediaSpace4>private');
...
//var_dump($kal_category);
    ...
    $privilege = array(
        'app' => 'moodle',
        'userId' => $username,
        'returnUrl' => $CFG->wwwroot . '/local/kaltura/reports.php?courseid=' . $courseid,
...
function local_kaltura_get_kcw($type = 'res_uploader', $admin = false) {
    ...
    $vars       = local_kaltura_get_flashvars($client_obj, $kal_user, $type);
...
/**
 * Return the default category for uploads
 *
 * @return string - category
 */
function local_kaltura_get_category() {
    return get_config(KALTURA_PLUGIN_NAME, 'res_default_category');
}

/**
 * Return default tags to add as metadata for uploads
 *
 * @return string - tags
 */
function local_kaltura_get_tags($type) {
	global $COURSE;

	$tags = $COURSE->shortname .',' .$COURSE->fullname;
	if ($type == 'pres_uploader') {
		$tags .= ',video_presentation';
    	} elseif ($type == 'assign_uploader') {
		$tags .= ',video_assignment';
	} else {
		$tags .= ',video_resource';
    	}
    	return $tags;
 ...
 * @return string - querystring of flashvars
 */
function local_kaltura_get_flashvars($client_obj, $user_obj, $type = 'res_uploader', $params = array()) {
    ...
    // Added by m.stubbs@mmu.ac.uk 2013-01-12
    $vars['enforceTags'] = local_kaltura_get_tags($type);
    $vars['disableCategories'] = true;
    $vars['categoriesRootId'] = get_config(KALTURA_PLUGIN_NAME, 'res_categories_root_id'); // '6404482';
    $vars['enforceCategory'] = local_kaltura_get_category(); // 'MediaSpace>private'
